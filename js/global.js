(function ($) {
  ...
  try {
    ...
    preloader_object.animsition({
      ...
      outDuration: 800,
      linkElement:
        'a:not([target="_blank"]):not([href^="#"]):not([class^="chosen-single"]):not([data-js-animation^="no-animate"])',
      loading: true,
...
(function ($) {
  ...
  try {
    ...
    $(".menu-mobile li.menu-item.  > a").on("click", function () {
  ...
  if (leadForm.length) {
    leadForm.on("submit", function (e) {
      ...
      $.ajax({
        ...
        error: function () {
          $(".form-message").html(
            '<div class="alert alert-danger">Wystąpił problem z wysyłaniem wiadomości. Spróbuj jeszcze raz.</div>'
          );
        },
        complete: function () {
          $("#lead-form")[0].reset();
